...
const Room2 = () => {
  ...
  useEffect(() => {
    const initRoom = async () => {
      try {
        ...
        const res = await axios.post("https://escape-the-sector.onrender.com/api/game/start-room2", {}, {
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      setGameOver(true);
      const token = sessionStorage.getItem('token');
      axios.post("https://escape-the-sector.onrender.com/api/game/fail-room", { roomNumber: 2 }, {
        headers: { Authorization: `Bearer ${token}` }
  ...
  const handleDoorSelect = async (mode) => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      await axios.post("https://escape-the-sector.onrender.com/api/game/start-room2", {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (err) {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    if (finalInput.trim() === expectedFinal) {
      ...
      try {
        await axios.post(
          "https://escape-the-sector.onrender.com/api/game/complete-room2",
          {
            door: doorMode
          },
          {
