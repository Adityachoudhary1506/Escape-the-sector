...
const AdminDashboard = () => {
  ...
  const fetchRoom2Leaderboard = async () => {
    try {
      ...
      const res = await axios.get("https://escape-the-sector.onrender.com/api/admin/room2-leaderboard", {
  ...
  const fetchFull = async () => {
    try {
      const token = sessionStorage.getItem('token');
      const res = await axios.get("https://escape-the-sector.onrender.com/api/admin/full-progress", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchFinal = async () => {
    try {
      const token = sessionStorage.getItem('token');
      const res = await axios.get("https://escape-the-sector.onrender.com/api/admin/final-leaderboard", {
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            <button
              ...
              onClick={async () => {
                if (window.confirm(`Are you sure you want to unlock Room ${room} globally?`)) {
                  try {
                    const token = sessionStorage.getItem('token');
                    await axios.post("https://escape-the-sector.onrender.com/api/admin/start-room", 
                      { roomNumber: room },
                      { headers: { Authorization: `Bearer ${token}` } }
                    );
